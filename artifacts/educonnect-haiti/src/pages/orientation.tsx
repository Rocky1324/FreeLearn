...
export default function Orientation() {
  ...
  const calculateResults = (finalAnswers: Record<number, number>) => {
    ...
    const topFamilies = Object.entries(scores).sort((a, b) => b[1] - a[1]).slice(0, 3).map(entry => entry[0]);
      ...
      <Layout>
        <div className="bg-primary text-primary-foreground py-16 text-center">
          <div className="container mx-auto px-4">
            ...
            <h1 className="text-4xl md:text-5xl font-bold font-serif mb-4">{t.orientation.profileTitle}</h1>
            <p className="text-xl opacity-80 max-w-2xl mx-auto">{t.orientation.profileSubtitle}</p>
            <div className="mt-8">
                  ...
                  <div className="space-y-4 pt-6 border-t">
                    ...
                    <Link href="/cours">
                      <Button variant="link" className="px-0 text-primary group">
                        {t.orientation.prepCourses} <ArrowRight className="ml-1 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                      </Button>
                    </Link>
                  </div>
                </div>
        ...
        {!started ? (
          <div className="text-center space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-4">
              <h1 className="text-4xl md:text-5xl font-bold font-serif">{t.orientation.heroTitle}</h1>
          ...
          <div className="space-y-8 animate-in fade-in zoom-in-95 duration-300">
            <div className="space-y-2">
              <div className="flex justify-between text-sm font-medium text-muted-foreground">
                <span>Question {currentStep + 1} {t.orientation.questionOf} {quizQuestions.length}</span>
