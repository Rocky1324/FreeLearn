...
function buildMarkerIcon(color: string) {
  return L.divIcon({
    ...
    html: `<div style="background:${color};width:22px;height:22px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.4);"></div>`,
...
export default function Schools() {
  ...
  const geocode = async (q: string) => {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        setUserLocation({ lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon), label: data[0].display_name || q });
  ...
  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); if (!address.trim()) return; geocode(address.trim()); };
  ...
  const handleGeolocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setUserLocation({ lat: pos.coords.latitude, lng: pos.coords.longitude, label: t.schools.myLocation });
        setSearching(false);
        toast.success("Position détectée");
      },
      (err) => { console.error(err); toast.error("Impossible d'obtenir votre position."); setSearching(false); },
    ...
    <Layout>
      <div className="bg-muted/30 py-10 border-b">
        <div className="container mx-auto px-4">
          <h1 className="text-3xl md:text-4xl font-bold font-serif mb-2">{t.schools.title}</h1>
          <p className="text-muted-foreground max-w-2xl">{t.schools.subtitle}</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <Button type="button" variant="outline" className="h-12" onClick={handleGeolocate} disabled={searching}>
            ...
            {t.schools.myLocation}
          </Button>
        </form>

        <div className="flex flex-wrap items-center gap-2 mb-4 text-sm">
          <span className="text-muted-foreground mr-2">{t.schools.filterLabel}</span>
          {([
            { v: "all", label: t.schools.allFilter, color: "#1e293b" },
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm transition-colors ${
                statusFilter === f.v ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-muted border-border"
              }`}
            >
        ...
        <div className="bg-blue-50 border border-blue-200 text-blue-900 rounded-lg p-3 mb-4 text-sm flex gap-2">
          ...
          <span>{t.schools.disclaimer}</span>
        </div>

        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 h-[500px] rounded-2xl overflow-hidden border shadow-sm">
            <MapContainer center={HAITI_CENTER} zoom={8} style={{ height: "100%", width: "100%" }} scrollWheelZoom={true}>
              <TileLayer
              ...
              {userLocation && (
                ...
                >
                  <Popup>
                    <strong>{t.schools.youAreHere}</strong><br />
                ...
                <Marker key={s.id} position={[s.lat, s.lng]} icon={ICONS[s.status]}>
                  <Popup>
                    <div className="space-y-1 min-w-[180px]">
                      ...
                      <div className="text-xs text-muted-foreground">{s.city} · {s.department}</div>
                      <div className="text-xs">{s.level} · {s.type}</div>
                      <div
                      ...
                      {s.statusNote && <div className="text-xs italic text-muted-foreground mt-1">{s.statusNote}</div>}
                    </div>
                  </Popup>
                </Marker>
              ))}
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
            <h2 className="font-bold text-lg sticky top-0 bg-background py-2">
              {userLocation ? t.schools.closestSchools : t.schools.allSchools}
              <span className="ml-2 text-sm font-normal text-muted-foreground">({sortedSchools.length})</span>
            </h2>
            {sortedSchools.length === 0 && (
              <p className="text-sm text-muted-foreground">{t.schools.noSchools}</p>
            )}
            {sortedSchools.map((s) => {
