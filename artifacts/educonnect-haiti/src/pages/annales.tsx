...
import { useGoogleDrive } from "@/hooks/use-google";
import {
  examPapers,
  examResources,
  upcomingExams,
...
function SaveToDriveButton({ res }: { res: ExamResource }) {
  const { uploadFileFromUrl, uploading } = useGoogleDrive();

  const handleSave = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const proxyUrl = `${API_BASE_URL}/api/proxy-pdf?url=${encodeURIComponent(res.url)}`;
    const fileName = `${res.subject} — ${res.title}.pdf`;
    try {
      await uploadFileFromUrl(proxyUrl, fileName, "application/pdf");
    } catch {
  ...
  return (
    <button
      onClick={handleSave}
      disabled={uploading}
      title="Enregistrer dans Google Drive"
      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all bg-blue-50 text-blue-700 hover:bg-blue-100 disabled:opacity-50"
    >
      {uploading ? <Loader2 className="w-3 h-3 animate-spin" /> : <GoogleDriveIcon />}
      Drive
    </button>
  );
}
