...
import { ChevronLeft, ChevronRight, Calendar, Plus, X, Clock, Bell, RefreshCw } from "lucide-react";
import { Layout } from "@/components/layout/Layout";
import { Button } from "@/components/ui/button";
import { useLocalStorage } from "@/hooks/use-local-storage";
import { courses } from "@/data/courses";
import { useLanguage } from "@/hooks/use-language";
import { useGoogleCalendar } from "@/hooks/use-google";
...
export default function CalendarPage() {
  const { t } = useLanguage();
  const { syncAcademicEvents, addStudySession, syncing } = useGoogleCalendar();
    ...
    if (course) {
      try {
        await addStudySession(
          `[FreeLearn] Révision — ${course.subject}`,
          `Session de révision de ${addDuration} min pour ${course.title}`,
          selected,
          selected,
        );
      } catch {
  ...
  const handleSyncAllToGoogle = async () => {
    const futureEvents = ACADEMIC_EVENTS.filter(e => e.date >= today.toISOString().split("T")[0]);
    try {
            ...
            {selected ? (
              <div className="bg-card border rounded-2xl p-6 space-y-5 sticky top-20">
                ...
                <div className="border-t pt-4 space-y-3">
                  ...
                  <Button className="w-full" onClick={addSession} disabled={syncing}>
                    <Plus className="w-4 h-4 mr-2" /> {t.calendar.add}
                  </Button>
                  <p className="text-[11px] text-muted-foreground text-center">
                    La session sera aussi ajoutée à Google Calendar si connecté
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="bg-card border rounded-2xl p-5">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    Synchronisez tous les événements scolaires avec votre Google Calendar pour ne rien manquer.
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
