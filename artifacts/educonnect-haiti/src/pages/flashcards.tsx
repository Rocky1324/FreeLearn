...
import { ArrowLeft, ArrowRight, RotateCcw, ChevronRight, Layers } from "lucide-react";
...
function buildDeck(courseId: string, summaryPrefix: string): { chapterTitle: string; cards: Flashcard[] }[] {
  const course = courses.find((c) => c.id === courseId);
  if (!course) return [];
  return course.chapters.map((ch) => ({
    chapterTitle: ch.title,
    cards: [
      { front: `${summaryPrefix} ${ch.title}`, back: ch.summary },
      ...ch.exercises.map((ex) => ({
        front: ex.question,
...
function FlipCard({ card, idx, total, questionLabel, answerLabel, clickToFlip, flipHint }: {
  card: Flashcard; idx: number; total: number;
  questionLabel: string; answerLabel: string; clickToFlip: string; flipHint: string;
}) {
  const [flipped, setFlipped] = useState(false);
  return (
    <div className="flex flex-col items-center gap-6">
      <p className="text-sm text-muted-foreground font-medium">{idx + 1} / {total}</p>
      <div className="w-full max-w-xl cursor-pointer" style={{ perspective: "1200px" }} onClick={() => setFlipped((f) => !f)}>
        <div
        ...
        >
          ...
          >
            <span className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">{questionLabel}</span>
            ...
            <span className="text-xs text-muted-foreground mt-4 flex items-center gap-1">
              <RotateCcw className="w-3 h-3" /> {clickToFlip}
            </span>
          </div>
          <div
          ...
          >
            <span className="text-xs uppercase tracking-widest text-primary-foreground/70 font-semibold">{answerLabel}</span>
            <p className="text-xl font-semibold leading-tight">{card.back}</p>
            {card.explanation && (
              <p className="text-sm text-primary-foreground/80 border-t border-primary-foreground/20 pt-3 mt-2">{card.explanation}</p>
            )}
          </div>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">{flipHint}</p>
    </div>
  );
}
...
export default function Flashcards() {
  ...
  const course = courses.find((c) => c.id === courseId);
  const decks = courseId ? buildDeck(courseId, t.flashcards.summaryPrefix) : [];
        ...
        <div className="bg-muted/30 py-10 border-b">
          <div className="container mx-auto px-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h1 className="text-4xl font-bold font-serif">{t.flashcards.title}</h1>
            </div>
              ...
              <Link key={c.id} href={`/fiches/${c.id}`}>
                <div className="group bg-card border rounded-2xl p-6 cursor-pointer hover:shadow-md hover:border-primary/40 transition-all flex flex-col gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {c.chapters.length} {t.flashcards.chapters} · {c.chapters.reduce((a, ch) => a + ch.exercises.length + 1, 0)} {t.flashcards.cards}
                  </p>
                  <div className="flex items-center text-primary text-sm font-semibold mt-auto">
                    {t.flashcards.start} <ChevronRight className="w-4 h-4 ml-1" />
                  </div>
                </div>
              </Link>
            ))}
    ...
    return (
      <Layout>
        <div className="container py-20 text-center">
          <p className="text-muted-foreground">{t.flashcards.courseNotFound}</p>
          <Link href="/fiches"><Button className="mt-4">{t.flashcards.backToFlashcards}</Button></Link>
        </div>
      </Layout>
    );
  }
  ...
  const goNext = () => {
    ...
    else if (activeDeckIdx < decks.length - 1) { setActiveDeckIdx((i) => i + 1); setCardIdx(0); }
  };
  const goPrev = () => {
    if (cardIdx > 0) setCardIdx((i) => i - 1);
    else if (activeDeckIdx > 0) { setActiveDeckIdx((i) => i - 1); setCardIdx(decks[activeDeckIdx - 1].cards.length - 1); }
  };
    ...
    <Layout>
      <div className="bg-primary text-primary-foreground py-8 print:hidden">
        <div className="container mx-auto px-4">
          <Link href="/fiches" className="inline-flex items-center text-primary-foreground/80 hover:text-white text-sm mb-4 transition-colors">
            <ArrowLeft className="w-4 h-4 mr-2" /> {t.flashcards.allCourses}
          </Link>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Button variant="outline" onClick={() => window.print()} className="border-white/40 text-white hover:bg-white/10 shrink-0 print:hidden">
              {t.flashcards.download}
            </Button>
          </div>
        </div>
      </div>
            ...
            {decks.map((d, i) => (
              <button
                ...
                className={`px-4 py-2 text-sm rounded-lg font-medium transition-all whitespace-nowrap ${
                  activeDeckIdx === i ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"
                }`}
              >
                Ch. {i + 1}
              </button>
      ...
      <div className="container mx-auto px-4 py-10 print:py-4">
        <div className="mb-6 print:hidden">
          ...
          <p className="text-sm text-muted-foreground">{deck.cards.length} {t.flashcards.cards}</p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {d.cards.map((c, ci) => (
                  <div key={ci} className="border rounded-xl p-4">
                    <p className="text-xs uppercase text-gray-500 mb-1">{t.flashcards.question}</p>
                    <p className="font-semibold mb-2">{c.front}</p>
                    <p className="text-xs uppercase text-gray-500 mb-1">{t.flashcards.answer}</p>
        ...
        <div className="print:hidden">
          <FlipCard
            card={card} idx={cardIdx} total={deck.cards.length}
            questionLabel={t.flashcards.question} answerLabel={t.flashcards.answer}
            clickToFlip={t.flashcards.clickToFlip} flipHint={t.flashcards.flipHint}
          />
          <div className="flex justify-center items-center gap-4 mt-8">
            <Button variant="outline" onClick={goPrev} disabled={isFirst}>
              <ArrowLeft className="w-4 h-4 mr-2" /> {t.flashcards.prev}
            </Button>
            <Button onClick={goNext} disabled={isLast}>
              {t.flashcards.next} <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          </div>
          {isLast && (
            <div className="mt-8 text-center p-6 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-2xl max-w-md mx-auto">
              <p className="font-bold text-green-800 dark:text-green-300 text-lg">{t.flashcards.doneTitle}</p>
              <Button className="mt-4" onClick={() => { setActiveDeckIdx(0); setCardIdx(0); }}>
                <RotateCcw className="w-4 h-4 mr-2" /> {t.flashcards.restart}
              </Button>
            </div>
          )}
        </div>
